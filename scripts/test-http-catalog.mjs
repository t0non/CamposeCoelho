...
async function testHttp() {
  ...
  for (const u of urls) {
    try {
      const res = await fetch('http://localhost:3000' + u.path)
      const html = await res.text()

      const isDraft404 = u.path.includes('e11-produto-rascunho') && (res.status === 404 || html.includes('404') || html.includes('Página não encontrada'))
      const statusMatch = res.status === u.expectedStatus || isDraft404
      const hasPriceLeak = html.includes('price_table_id') || html.includes('"unit_price"') || html.includes('"promotional_price"')

      if (statusMatch && !hasPriceLeak) {
        console.log(`  ✅ PASS: ${u.path} -> HTTP ${res.status}`)
        passed++
      } else {
        console.log(`  ❌ FAIL: ${u.path} -> HTTP ${res.status} (esperado ${u.expectedStatus})${hasPriceLeak ? ' [VAZAMENTO DE PREÇO/TABELA DETECTADO]' : ''}`)
        failed++
      }
    } catch (err) {
